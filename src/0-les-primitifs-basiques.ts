...
export function testReferences() {
   ...
   * Tuples
   */
  const tuple: Array<string | number> = ["Alice", 25];
...
export function testGenericsTypes() {
   ...
   */
  function addProp<T extends Record<string, string>>(input: T): T & { newProp: string } {
   ...
   */
  function transform<T, R>(input: T, fn: (input: T) => R): R {
    return fn(input)
  }
   ...
   */
  function lookup<T, K extends keyof T>(data: T, prop: K) {
    return data[prop]
  }
...
export function testLiteralTypes() {
  ...
  type ExtractFeeling<Statement> =
    Statement extends `${infer _firstPerson} ${infer Feeling} ${infer _secondPerson}`
    ? Feeling
    : "";
